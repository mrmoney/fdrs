...
mini.extend(UserControl.MultiUpload, mini.DataGrid, {
    ...
    _create: function () {
        ...
        var sb = [];
        //sb.push('<table><tr><td>');
        sb.push('<table width="100%"><tr><td style="height:25px;"><a class="mini-button" iconCls="icon-search">选择文件...</a><span class="mini-upload"><span id="' + 
                me._uploadId + '" class="mini-upload-placeholder" style=""></span></span></td>');
        sb.push('<td style="display:none"><a class="mini-button" iconCls="icon-upload" name="multiupload">批量上传</a></td>');
        sb.push('<td style="display:none"><a class="mini-button" iconCls="icon-remove" name="removeAll">清空上传</a></td>');
        //sb.push('</tr></table>');
        //sb.push('</table></td>');
        sb.push('<td align="right">* 要上传的文件</td></tr></table>');
    ...
    bindEvents: function () {
        ...
        $(document.body).on("click", ".upicon-remove", function () {
            ...
            if (me.uploader && me.uploader.getStats().files_queued !== 0) {
            ...
            if(row.status == 1){
                ...
                showloading('删除中...');
                $.ajax({
                    url: '/?app=' + APP + '&act=remove_uploding',
                    data: row,type:'post',
    ...
    __on_upload_success: function (file, serverData) {
        var json = mini.decode(serverData);
        var me = this;
        var row = me.findRow(function (row) {
            if (row.fileId == file.id) return true;
        });
        //console.log(row);
        if(json.code == 1){
            me.updateRow(row, { status: 1, complete: 100,srv_data:json.data });
    ...
    isAllUploadOver:function(){
        /*判断是否已经全部上传完成*/
        var me = this;
        var row = me.findRow(function (row) {
            if (row.status == 0) return true;
        });
        //console.log('isAllUploadOver -> ',row);
    ...
    getAttrs: function (el) {
        ...
        mini._ParseString(el, attrs,
            ["uploadUrl", "flashUrl", "limitType", "limitSize", "uploadName", 
                "queueLimit", "onuploaderror", "onuploadsuccess"]
        );
