...
function SendUploadResults( $errorNumber, $fileUrl = '', $fileName = '', $customMsg = '',$oriFileName = '' )
{
	//暂停此功能，请重新完善...
	//$suffix_filename=str_replace('/attachement/fromeditor/','',$fileUrl);
	//$suffix_filename = substr(md5(time()),0,8) . base64_encode($suffix_filename);
	//$fileUrl= '/download/?do=downfromeditor&file=' . $suffix_filename;
...
EOF;
	...
	$rpl = array( '\\' => '\\\\', '"' => '\\"' ) ;
	if($oriFileName=='')$oriFileName=$fileName;
	echo 'window.parent.OnUploadCompleted(' . $errorNumber . ',"' . strtr( $fileUrl, $rpl ) . '","' . strtr( $oriFileName, $rpl ) . '", "' . strtr( $customMsg, $rpl ) . '") ;' ;
	echo '</script>' ;
	exit ;
}
