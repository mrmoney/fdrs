...
class clsMsDocGenerator
{
        ...
        * @param $classesName: array with class name of any class defined in extra format file, a item for each cell item
         */
        function addTableRow($cells, $aligns = NULL, $vAligns = NULL, $inlineStyle = NULL, 
							 $classesName = NULL, $colSpan=NULL){
                if(! $this->tableIsOpen)
                ...
                for($i = 0; $i < count($cells); $i++){
                        ...
                        $this->documentBuffer .= "<td width=$cellWidth align=$align valign=$vAlign style='$style' ".($className != '' ? " class=$className" : '') . ($colSpan!=''	?	" colspan=$colSpan" :	'') .">$cells[$i]</td>\n";
                }
