...
class ImageWatermark{  
    ...
    public $fontFile = '/inc/simhei.ttf';   //字体文件名   
    ...
    public function appendTextMark($markText){  
        ...
        $box = imagettfbbox($this->fontSize,$this->angle,ROOT . $this->fontFile,$markText);  
        ...
        imagettftext($this->dest_res,$this->fontSize,$this->angle,$pos[0],$pos[1],$imageColor,ROOT . $this->fontFile,$markText);  
    ...
    public function appendImageMark($markImage){  
        ...
        $pos = ($this->markPos!=null)?$this->markPos:$this->getMarkPosition($this->markPosType,false);  
    ...
    private function getMarkPosition($type=0,$is_txt=true)
	{  
  		$default_diff_num=5;
        switch($type)
		{  
            ...
            case 2: $x = ($this->dest_width-$this->mark_width)/2;  
                    $y =$is_txt	? $default_diff_num	:	0;  
                    break; //topcenter   
  
            case 3: 
					if($is_txt)
					{
						$x = $this->dest_width-$this->mark_width - $default_diff_num;  
						$y = $default_diff_num;  
					}
					else
					{
            ...
            case 4: $x = $is_txt	?	$default_diff_num	:	0;  
            ...
            case 7: 
					if($is_txt)
					{
						$x = $default_diff_num; $y = $this->dest_height-$this->mark_height -$default_diff_num;  
					}
					else
					{
						$x = 0; $y = $this->dest_height-$this->mark_height;  
					}
                    break;//bottomleft   
  
            case 8: 
					$x_num=$is_txt	?	-$default_diff_num	:	0;
					$x = ($this->dest_width-$this->mark_width)/2 + $x_num;  
                    $y = $this->dest_height-$this->mark_height + $x_num;  
                    break;//bottomcenter   
            ...
            case 9: 
					...
					{
						$x = $this->dest_width-$this->mark_width -$default_diff_num;  
						$y = $this->dest_height-$this->mark_height -$default_diff_num;  
					}
					else
					{
						$x = $this->dest_width-$this->mark_width;  
